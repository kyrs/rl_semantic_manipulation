...
class Model(tf.Module):
    ...
    def __init__(self, *, ac_space, policy_network, value_network=None, ent_coef, vf_coef, max_grad_norm):
        ...
        self.kl_unit_norm_coeff = 0.01 ### see a better way of adding similar to other coefficients
        ...
        self.unit_normal_dist = DiagGaussianPd(tf.tf.zeros(self.train_model.pdtype.param_shape())) ### diagonalgaussianPD is different from pdtype it take a list of mean and logStd
        if MPI is not None:
    ...
    def get_grad(self, cliprange, obs, returns, masks, actions, values, neglogpac_old):
        ...
        with tf.GradientTape() as tape:
            ...
            ################################ part where loss is defined ####################
            kl_loss_val = pd.kl(self.unit_normal_dist) ## taking kl divergence NOTE : KL DIVEGENCE IS NOT commutative 
            loss = pg_loss - entropy * self.ent_coef + vf_loss * self.vf_coef + self.kl_unit_norm_coeff * kl_loss_val
